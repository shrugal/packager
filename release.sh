...
usage() {
	echo "Usage: release.sh [-celzdLCWHGosui] [-t topdir] [-r releasedir] [-p curse-id] [-w wowi-id] [-w wowi-id-test] [-j github-slug] [-k gitlab-slug] [-g game-version] [-m pkgmeta.yml]" >&2
...
OPTIND=1
while getopts ":celzdLCWHGosuit:r:p:w:b:j:k:g:m:" opt; do
	case $opt in
	c)
...
if [ -n "$manual_changelog" ] && [ -f "$topdir/$changelog" ]; then
	...
	if [ "$repository_type" = "git" ]; then
		changelog_url=
		changelog_version=
		changelog_previous="[Previous Releases](${githost_url}/releases)"
		changelog_url_wowi=
...
if [ -z "$skip_upload" ]; then
	...
	if [[ -n "$upload_curseforge" || -n "$upload_wowinterface" || -n "$upload_github" || -n "$upload_gitlab" ]] && ! jq --version &>/dev/null; then
	...
	if [[ -n "$upload_github" || -n "$upload_gitlab" ]]; then
		...
		if [ -n "$upload_github" ]; then
			upload_github_asset() {
				...
				asset_id=$( curl -sS -H "Authorization: token $github_token" "https://api.github.com/repos/$github_slug/releases/$_ghf_release_id/assets" | jq '.[] | select(.name? == "'"$_ghf_file_name"'") | .id' )
				if [ -n "$asset_id" ]; then
			...
			release_id=$( curl -sS -H "Authorization: token $github_token" "https://api.github.com/repos/$github_slug/releases/tags/$tag" | jq '.id // empty' )
			if [ -n "$release_id" ]; then
				echo "Updating GitHub release: https://github.com/$github_slug/releases/tag/$tag"
				_gh_release_url="-X PATCH https://api.github.com/repos/$github_slug/releases/$release_id"
			else
				echo "Creating GitHub release: https://github.com/$github_slug/releases/tag/$tag"
				_gh_release_url="https://api.github.com/repos/$github_slug/releases"
			fi
