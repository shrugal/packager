...
In addition, the following environmental variables are accepted:
  ...
  - `GH_SLUG` - a GitHub project slug, if different from Git remote URL.
  - `GITHUB_OAUTH` - a [GitHub personal access token](https://github.com/settings/tokens), **required** for uploading to GitHub.
  - `GL_SLUG` - a GitLab project slug, if different from Git remote URL.
  - `GITLAB_TOKEN` - a [GitLab personal access token](https://gitlab.com/profile/personal_access_tokens), **required** for uploading to GitLab.
